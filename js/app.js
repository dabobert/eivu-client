  ...
  testUpload = function() {
    ...
    Folder.traverse(initialDir, function(fullPath, fileStats) {
      var md5Promise = new Promise((resolve, reject) => md5File(fullPath, function (error, md5) {
        ...
        var tester = CloudFile.toFilename(fullPath);
        console.log(tester)
        fileData.push({ fullPath: fullPath, md5: md5, filename: tester, size: fileStats.size })
        $('<tr id="' + md5 + '"><td>' + tester + '</td><td>' + fileStats.size + '</td><<td>' + md5 + '</td><td>Queued</td></tr>').appendTo('table#fileData tbody');
      resolve(md5);
      }));//end md5File
